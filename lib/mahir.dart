...
class _ResultState extends State<Result> {
  int? previousScore; // Added to store the previous score
  ...
  Future<void> getPreviousScore() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      final userScoresRef =
      FirebaseFirestore.instance.collection('users').doc(user.uid);

      final docSnapshot = await userScoresRef.get();
      if (docSnapshot.exists) {
        final data = docSnapshot.data() as Map<String, dynamic>;
        final previousScore = data['scores'] as int?;
        setState(() {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          if (previousScore != null) // Display previous score if available
            const SizedBox(height: 16.0),
          Text(
